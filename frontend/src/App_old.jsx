// Clean App.jsx Imports - Ready for StatePage Component
...
// Lucide Icons - Complete set for all components including StatePage
import {
  // Layout & Navigation
  Building,
  GraduationCap,
  ...
  // StatePage specific icons
  BarChart3,        // For potential impact analysis
  MessageCircle,    // For AI talking points
  Mail,
  Phone,
  HelpCircle,
  Monitor,
...
// Complete StatePage Component with Working Highlights
const StatePage = ({ stateName, stableHandlers, copyToClipboard }) => {
  ...
  const [isFetchExpanded, setIsFetchExpanded] = useState(false);
  const [fetchingData, setFetchingData] = useState(false);
  const [fetchMethod, setFetchMethod] = useState('search');
  ...
  // CRITICAL: Custom getOrderId function for state bills to match the global one
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <div className="p-6">
            {stateLoading ? (
              ...
              <div className="space-y-6">
                {/* ✅ FIXED: Bills mapping with working highlight functionality */}
                {filteredStateOrders.map((bill, index) => {
                  ...
                  return (
                    <div key={bill.id || index} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-300">
                      {/* Bill Header */}
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {/* ✅ WORKING: Highlight button that matches Executive Orders style */}
          ...
          <Routes>
            ...
            {Object.keys(SUPPORTED_STATES).map(state => (
              <Route 
                ...
                element={<StatePage stateName={state} stableHandlers={stableHandlers} copyToClipboard={copyToClipboard} />}
              />
            ))}
