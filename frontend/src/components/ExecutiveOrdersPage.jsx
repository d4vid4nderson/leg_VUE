...
// Valid categories: civic, education, engineering, healthcare, not-applicable
const cleanCategory = (category) => {
    ...
    // Valid frontend filter categories
    const validCategories = ['civic', 'education', 'engineering', 'healthcare', 'not-applicable'];
    ...
    // Map legacy/alternate category names to valid ones
    const categoryMap = {
        // Map to civic
        'government': 'civic',
        'government-operations': 'civic',
        'public policy': 'civic',
        'municipal': 'civic',
        'federal': 'civic',
        'administration': 'civic',
        'criminal-justice': 'civic',
        'crime': 'civic',
        'police': 'civic',
        'security': 'civic',
        'national-security': 'civic',
        'economics': 'civic',
        'trade': 'civic',
        'finance': 'civic',
        'economic': 'civic',
    ...
    return categoryMap[trimmedCategory] || 'not-applicable';
};
...
const ExecutiveOrdersPage = ({ stableHandlers, copyToClipboard }) => {
  ...
  const filterCounts = useMemo(() => {
    ...
    counts.all_practice_areas = counts.civic + counts.education + counts.engineering + counts.healthcare;
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (categoryFilters.length > 0) {
      // Special handling for all_practice_areas - it includes civic, education, engineering, healthcare (excludes not-applicable)
      const allPracticeAreasCategories = ['civic', 'education', 'engineering', 'healthcare'];

      result = result.filter(order => {
        const orderCategory = order?.category;

        // If all_practice_areas is selected, match any of the 4 practice area categories
        if (categoryFilters.includes('all_practice_areas')) {
          if (allPracticeAreasCategories.includes(orderCategory)) {
            return true;
          }
        }

        // Also check if the order's specific category is in the selected filters
        return categoryFilters.includes(orderCategory);
      });
