  ...
  const filteredStateOrders = useMemo(() => {
    ...
    if (selectedFilters.length > 0) {
      ...
      filtered = filtered.filter((bill) => {
        const billCategory = cleanCategory(bill?.category);
        ...
        if (selectedFilters.includes('all_practice_areas')) {
          if (allPracticeAreasCategories.includes(billCategory)) {
            return true;
          }
        }

        // Also check if the bill's specific category is in the selected filters
        return selectedFilters.includes(billCategory);
      });
    }
